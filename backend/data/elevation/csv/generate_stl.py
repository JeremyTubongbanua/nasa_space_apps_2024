...
def generate_stl(csv_files, lng, lat, output_stl):
    ...
    all_lons, all_lats, all_elevations = [], [], []

    # Load and merge data from all CSV files
    for csv_file in csv_files:
        lons, lats, elevations = load_csv_data(csv_file, lng, lat)
    ...
    # Normalize coordinates and elevations
    ...
    if np.max(all_elevations) - np.min(all_elevations) == 0:
        z_normalized = np.zeros_like(all_elevations)
    else:
        z_normalized = (all_elevations - np.min(all_elevations)) / (np.max(all_elevations) - np.min(all_elevations)) * 30
